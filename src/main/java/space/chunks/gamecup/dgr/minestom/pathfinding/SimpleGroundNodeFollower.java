 ...
 * @author Nico_ND1
 */
public class SimpleGroundNodeFollower implements NodeFollower {
  private final Entity entity;

  public SimpleGroundNodeFollower(@NotNull Entity entity) {
    this.entity = entity;
  }

  /**
   * Used to move the entity toward {@code direction} in the X and Z axis Gravity is still applied but the entity will not attempt to jump Also update the yaw/pitch of the entity to look along
   * 'direction'
   *
  ...
  public void moveTowards(@NotNull Point direction, double speed, @NotNull Point lookAt) {
    ...
    this.entity.refreshPosition(new Pos(speedX, 0, speedZ).add(this.entity.getPosition()).withView(yaw, pitch));
  }
